...
public class Phase3Wiki {
    public static void main(String[] args) throws IOException {

        Word2Vec vec = WordVectorSerializer.readWord2VecModel("Phase 1/src/main/model.bin");
        ...
        readQueriesFromFile(filePath, queriesMap);
        ...
        List<Map.Entry<String, Double>> sortedDocs;

        try (FileWriter writer = new FileWriter("Phase 1/src/main/trec_eval/results3.txt")) {
    ...
    private static double cosineSimilarity(INDArray vec1, INDArray vec2) {
        return Nd4j.getExecutioner().execAndReturn(new org.nd4j.linalg.api.ops.impl.reduce3.CosineSimilarity(vec1, vec2)).getFinalResult().doubleValue();
    }
    ...
    public static INDArray toDenseAverageVector(Word2Vec word2Vec, String... terms) {
        ...
        for (String term : terms) {
            if (word2Vec.hasWord(term)) {
                vectors.add(word2Vec.getWordVectorMatrix(term));
            }
        }
        if (vectors.isEmpty()) {
            return null; // Return null if no terms have vectors in the model
        }
        return Nd4j.vstack(vectors).mean(0);
    }

    private static void readQueriesFromFile(String filePath, Map<String, List<String>> queriesMap) {
